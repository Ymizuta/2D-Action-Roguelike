...
public class Path : MonoBehaviour
{
	...
	{
		if (!isStart && transform.hasChanged)
		{
			currentPosition = transform.position;
		}
		...
		{
			Gizmos.DrawWireSphere(currentPosition + path[i], 1f);
			if (i < path.Count - 1)
			{
				Gizmos.DrawLine(currentPosition + path[i], currentPosition + path[i + 1]);
			}
			Gizmos.DrawLine(currentPosition + path[0], currentPosition + path[path.Count - 1]);
		}
	}
}
